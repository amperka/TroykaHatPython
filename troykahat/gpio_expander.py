...
class GpioExpander(object):
    ...
    def pinMode(self, pin, mode):
        ...
        if mode == Mode.INPUT:
            self._i2c.writeReg16(self._io, commands.PORT_MODE_INPUT, data)
        elif mode == Mode.INPUT_PULLUP:
            self._i2c.writeReg16(self._io, commands.PORT_MODE_PULLUP, data)
        elif mode == Mode.INPUT_PULLDOWN:
            self._i2c.writeReg16(self._io, commands.PORT_MODE_PULLDOWN, data)
        elif mode == Mode.OUTPUT:
            self._i2c.writeReg16(self._io, commands.PORT_MODE_OUTPUT, data)
    ...
    def digitalWrite(self, pin, value):
        data = self._reverse_uint16(1 << pin)
        if value:
            self._i2c.writeReg16(self._io, commands.DIGITAL_WRITE_HIGH, data)
        else:
            self._i2c.writeReg16(self._io, commands.DIGITAL_WRITE_LOW, data)
    ...
    def _digitalReadPort(self):
        port = self._reverse_uint16(
            self._i2c.readReg16(self._io, commands.DIGITAL_READ)
        )
        return port
    ...
    def _analogRead16(self, pin):
        ...
        return self._reverse_uint16(self._i2c.readReg16(
                                    self._io, commands.ANALOG_READ))

    def _setPwmFreq(self, freq):
        self._i2c.writeReg16(self._io, commands.PWM_FREQ,
